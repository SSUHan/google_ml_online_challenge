...
class CnnDiscriminator(BaseModel):
  def create_model(self, input_size, **unused_params):
    ...
    self.D_W5 = tf.Variable(xavier_init([12544, 1]), name='d/w5')
  ...
  def run_model(self, model_input, is_training=True, **unused_params):
    ...
    print("as list : ", self.D_W5.get_shape().as_list())
  
    net = tf.reshape(net, [-1, self.D_W5.get_shape().as_list()[0]])
  ...
  def get_variables(self):
    return [self.D_W1, self.D_W2, self.D_W3, self.D_W5, self.D_b1, self.D_b2, self.D_b3, self.D_b5]