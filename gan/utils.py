...
def AddGlobalStepSummary(summary_writer,
  ...
  """
  accuracy_on_fake = global_step_info_dict["accuracy_on_fake"]
  accuracy_on_real = global_step_info_dict["accuracy_on_real"]
  G_loss = global_step_info_dict["G_loss"]
  D_loss = global_step_info_dict["D_loss"]
  examples_per_second = global_step_info_dict.get("examples_per_second", -1)

  summary_writer.add_summary(
      MakeSummary("GlobalStep/" + summary_scope + "_D_accuracy_on_G",
          accuracy_on_fake), global_step_val)
  summary_writer.add_summary(
      MakeSummary("GlobalStep/" + summary_scope + "_D_accuracy_on_real",
          accuracy_on_real), global_step_val)
  summary_writer.add_summary(
      MakeSummary("GlobalStep/" + summary_scope + "_G_loss", G_loss),
      global_step_val)
  summary_writer.add_summary(
      MakeSummary("GlobalStep/" + summary_scope + "_D_loss", D_loss),
      global_step_val)
  ...
  info = ("global_step {0} | Batch D_acc_on_G: {1:.3f} | "
          "Batch D_acc_on_real: {2:.3f} | Batch G_loss: {3:.3f} | "
          "Batch D_loss: {4:.3f} | Examples_per_sec: {5:.3f}").format(
              global_step_val, accuracy_on_fake, accuracy_on_real,
              G_loss, D_loss, examples_per_second)
  return info
...
def AddEpochSummary(summary_writer,
  ...
  avg_accuracy_on_fake = epoch_info_dict["avg_accuracy_on_fake"]
  avg_accuracy_on_real = epoch_info_dict["avg_accuracy_on_real"]
  avg_G_loss = epoch_info_dict["avg_G_loss"]
  avg_D_loss = epoch_info_dict["avg_D_loss"]

  summary_writer.add_summary(
      MakeSummary("Epoch/" + summary_scope + "_Avg_D_accuracy_on_G",
          avg_accuracy_on_fake), global_step_val)
  summary_writer.add_summary(
      MakeSummary("Epoch/" + summary_scope + "_Avg_D_accuracy_on_real",
          avg_accuracy_on_real), global_step_val)
  summary_writer.add_summary(
      MakeSummary("Epoch/" + summary_scope + "_Avg_G_loss", avg_G_loss),
      global_step_val)
  summary_writer.add_summary(
      MakeSummary("Epoch/" + summary_scope + "_Avg_D_loss", avg_D_loss),
      global_step_val)
  summary_writer.flush()

  info = ("epoch/eval number {0} | Avg_D_acc_on_G: {1:.3f} | "
          "Avg_D_acc_on_real: {2:.3f} | Avg_G_loss: {3:.3f} | "
          "Avg_D_loss: {4:.3f}").format(
              epoch_id, avg_accuracy_on_fake, avg_accuracy_on_real, avg_G_loss,
              avg_D_loss)
  return info
